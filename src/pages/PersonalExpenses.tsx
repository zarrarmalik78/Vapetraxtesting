...
const PersonalExpenses: React.FC = () => {
  ...
  const categories = ['Groceries', 'Home Utilities', 'Rent', 'Personal Care', 'Fuel/Transport', 'Family', 'Shopping', 'Miscellaneous'];
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      selectedIds.forEach((id) => batch.delete(doc(db, 'personal_expenses', id)));
      await batch.commit();
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Personal Budget</h1>
            <p className="text-slate-500 text-sm">Track your personal and home spending entirely isolated from the shop</p>
          </div>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="metric-card-red rounded-2xl p-6 shadow-lg shadow-rose-500/20 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest mb-1">Total Personal Spend</p>
...
const AddPersonalExpenseModal: React.FC<{ categories: string[], expense?: any, onClose: () => void }> = ({ categories, expense, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        await updateDoc(doc(db, 'personal_expenses', expense.id), {
          ...formData,
        ...
        toast.success('Personal expense updated');
      } else {
